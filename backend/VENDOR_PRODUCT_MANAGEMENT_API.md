...
✅ Can be edited/updated before approval  
...
```json
{
    ...
    "title": "Updated Organic Face Wash",
    "status": "pending",
    "message": "Product updated. Status reset to pending for admin review."
}
```

**Automatic Status Reset:**
- If product was previously **approved** and is edited → status resets to **pending**
- If product was **pending** → remains pending
- If product was **rejected** → remains rejected (vendor must contact admin)

**Features:**
✅ Only vendor who owns product can edit  
✅ Auto-resets approval status if product was approved  
...
const updateProduct = async (productId, updates) => {
  const response = await fetch(`http://localhost:8000/api/vendor/products/${productId}/edit/`, {
    method: 'PUT',
    headers: {
